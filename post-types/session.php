...
function session_admin() {
  ...
  add_meta_box('session_no_event_meta_box',
               "Check this box if this is a non-event, ie breakfast, break, lunch",
               'display_session_no_event_meta_box',
               'session',
               'side',
               'high');
}
...
function display_session_no_event_meta_box($session) {
  global $post;
  $no_event = get_post_meta($session->ID, '_session_no_event', true);
  ?>
    <input type="checkbox" name="session-no-event" <?php if( $no_event == true ) { ?>checked="checked"<?php } ?>> This is a non-event.
  <?php
}
...
function add_session_fields($session_id, $session) {
  if($session->post_type == 'session') {
    ...
    // Update sessions without sessions attached.
    update_post_meta($session_id, '_session_no_event', $_POST['session-no-event']);
